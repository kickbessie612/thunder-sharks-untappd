...
class BeerForm(FlaskForm):
    ...
    abv = FloatField('ABV', validators=[DataRequired(), Length(max=255), NumberRange(min=0, max=100)])
    ibu = FloatField('IBU', validators=[DataRequired(), NumberRange(min=0, max=1000), NumberRange(min=0, max=1000)])
